...
from algorithm_hojman import check_convexity_polygon, sutherland_hojman
...
def close_figure(canvas, figure, figure_colour, fig_name):
    if len(figure) < 3:
        messagebox.showwarning("Ошибка ввода!", f"{fig_name} должен иметь >= 3 вершин!\n")
        return

    if figure[0] == figure[-1]:
        messagebox.showwarning("Ошибка ввода!", f"{fig_name} замкнут!\n")
        return
...
def cut_off(canvas, figure, clipper, result_colour):
    ...
    p, np = sutherland_hojman(figure, clipper)
