...
def line_by_line_filling_algorithm_with_seed(canvas, img, border_colour, fill_colour, seed_point, delay=False):
    ...
    while stack:
        ...
        while img.get(x, y) != fill_colour_rgb and img.get(x, y) != border_colour_rgb and x < CANVAS_WIDTH:
            draw_pixel(img, x, y, fill_colour)
            x += 1
        ...
        x = tx - 1
        while img.get(x, y) != fill_colour_rgb and img.get(x, y) != border_colour_rgb and x > 0:
            draw_pixel(img, x, y, fill_colour)
            x -= 1
        ...
        if ty < CANVAS_HEIGHT:
            ...
            while x <= xr:
                ...
                while img.get(x, y) != fill_colour_rgb and img.get(x, y) != border_colour_rgb and x <= xr:
                    flag = True
                    x += 1
